
import {useState, useEffect} from 'react' ; 


const Footer = () => {
	
	const [actionScroll , setactionScroll] = useState(false) ; 
	
	const scrollUp = () => {
		window.scrollTo({
			top : 0 , 
			behavior : 'smooth' , 
		}) ; 
	} ; 
	
	const changeScroll = () => {
		if(window.scrollY > 800) { setactionScroll(true) ;  }
		else { setactionScroll(false) ; }
	} ; 
	
	useEffect(() => {
		window.addEventListener('scroll' , changeScroll) ; 
		return () => window.removeEventListener('scroll' , changeScroll) ; 
	} , [actionScroll]) ; 
	
	
	return (
		<footer > 
			<div className="Footer" >
				<div className="container-footer" >
					<div className="connect-container" >
						<a href="#" >
							<img src="data:image/png;base64," alt="logo" /> 
						</a>
						<div className="group-connect" >
							<div className="email" >
								<i className="far fa-envelope-open" aria-hidden="true"></i>
								<span>khanhlatoi1998@gmail.com</span>
							</div>
							<div className="location-group"><i className="fas fa-search-location" aria-hidden="true">
								</i><span>Cần Thơ City</span>
							</div>
							<div className="location-group">
								<i className="fas fa-globe-americas" aria-hidden="true"></i><span>Cần Thơ City</span>
							</div>
						</div>
					</div>
					<div className="container-googlemap" >
						<iframe 
							src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d15678.066199265433!2d106.704559!3d10.7716931!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0xaa966d5f7d098671!2sTh%C3%A1p%20Bitexco%20Financial%20Tower!5e0!3m2!1svi!2s!4v1626947791133!5m2!1svi!2s" 
							style={{ height: `100%` , width: `100%` }}
							allowFullScreen="" 
							loading="lazy" >
						</iframe>
					</div>
				</div>
				<div className="ScroolUp" onClick={scrollUp} style={actionScroll === true  ? {display : "block"} : {display : "none"}} >
					<i className="fas fa-chevron-up" ></i>
				</div>
			</div>
		</footer>
	) ; 
} ; 

export default Footer ; 